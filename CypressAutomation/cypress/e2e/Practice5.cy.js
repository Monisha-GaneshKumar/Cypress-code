...
describe('Restful Booker POST & GET', () => {
  ...
  it('POST-Create Booking', function () {
    ...
    cy.request("POST", `${baseUrl}/booking`, Booking).then((response) => {
      ...
      expect(response.body.booking.firstname).to.eq('Moni');
      expect(response.body.booking.lastname).to.eq('Ganesh');
  ...
  it('GET-Get Booking data', function () {
    cy.request('GET', `${baseUrl}/booking/${bookingid}`).then((response) => {
      ...
      expect(response.body.firstname).to.eq('Moni');
      expect(response.body.lastname).to.eq('Ganesh');
...
it('GET-Get specific data', function () {
    cy.request('GET', `${baseUrl}/booking?firstname=Moni&lastname=Ganesh`).then((Response) => {  // validate by specific data "firstname=Moni&lastname=Ganesh"
        expect(Response.status).equal(200)   
